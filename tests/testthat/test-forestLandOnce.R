...
## Cache on and off (SpaDES' `spades.useCache = "eventsOnly"` turns every function-level Cache off).
##
...
.flResetMemo <- function() {
  e <- LandR:::.pkgEnv
  rm(list = grep("^forestLand", ls(e, all.names = TRUE), value = TRUE), envir = e)
}

.flRunDataYears <- function(dataYears, useCache) {
  withr::local_package("terra")
  srcDir <- withr::local_tempdir("flSrc_")
  dPath <- withr::local_tempdir("flInputs_")
  withr::local_options(list(
    reproducible.useCache = useCache,
  ...
  .flResetMemo()
  withr::defer(.flResetMemo())
  outs <- lapply(dataYears, function(y) {
    ## as fireSenseUtils::makeFireSenseLCC() calls it, under the module's Cache()
    out <- reproducible::Cache(
...
test_that("forest-land inputs are prepared once across dataYears (Cache off and on)", {
  ...
  for (useCache in c(FALSE, TRUE)) {
    res <- .flRunDataYears(dataYears, useCache = useCache)
    info <- paste("reproducible.useCache =", useCache)
