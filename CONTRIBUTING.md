...
### If you changed `calc_raster_counts()` or `prep_polygons()`

`vignettes/scanfi-summary.Rmd` reports exact national statistics for a 30 m
forest raster. Those cannot be computed during a build -- a single pass reads
tens of GB, CI runners have ~14 GB of disk, and the ecostratification polygon
host throttles CI IP ranges. So the one expensive step, the value-frequency
table from `calc_raster_counts()`, is precomputed and committed under
`inst/extdata/`; everything downstream (statistics, histograms, maps) is
recomputed at build time and cannot go stale.

The committed table *can* go stale, so it is pinned to a hash of the code that
produced it. If `tests/testthat/test-vignette-artifacts.R` reports

> The committed counts table in `inst/extdata/` was generated by different code
> than is in `R/` now
...
This needs the national rasters and takes a while; run it on a workstation, not
a laptop on hotel wifi. Commit the refreshed `inst/extdata/`. If you can't run
it, say so in the pull request and a maintainer will regenerate for you --
please don't edit the manifest by hand to silence the test, since that is
exactly the drift it exists to catch.

Note that changing `stats_from_counts()` or `weighted_quantile()` does **not**
require regeneration: the statistics are an exact function of the counts table
and are recomputed on every build.
